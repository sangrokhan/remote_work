"""Segmentation offload: parsing what ethtool says, putting back what we changed,
and the entrypoint-time bulk-disable knob -- merged from both projects' test suites.
...
# --- the capture-time window (env alias, default-off, precise restore) ----
...
def test_from_env_reads_canonical_disable_env(monkeypatch):
    monkeypatch.setenv("NIC_OFFLOAD_DISABLE", "1")
    monkeypatch.delenv("TRAFFIC_PCAP_NO_OFFLOAD", raising=False)
...
def test_from_env_reads_deprecated_alias_env(monkeypatch):
    """TRAFFIC_PCAP_NO_OFFLOAD is deprecated but must keep working for existing
    docker-compose.yml/docs that still set it."""
    monkeypatch.delenv("NIC_OFFLOAD_DISABLE", raising=False)
    monkeypatch.setenv("TRAFFIC_PCAP_NO_OFFLOAD", "1")
    cfg = offload.from_env()
    assert cfg["disable"] is True
