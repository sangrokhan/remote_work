...
Unchanged from `tcp_congestion/tcp_congestion/tcpinfo.py` -- token_traffic
has no equivalent (it relies on `cwnd.py`'s continuous netlink monitor
instead of one-shot snapshots), so this is a straight migration, not a
merge. Kept in `core/` rather than a backend-specific module because a
one-shot TCP_INFO read is a cheap, dependency-free alternative to the
netlink monitor and is useful to any backend, not just synthetic_mock.
"""
