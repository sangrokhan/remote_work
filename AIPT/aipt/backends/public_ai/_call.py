"""One turn on the wire, in one or two passes -- public_ai backend internals.

Ported from ``token_traffic/core/call.py`` (DESIGN.md 5, A2). Kept private
to ``aipt.backends.public_ai`` (leading underscore) rather than promoted to
``aipt/core`` -- unlike ``wire.py``/``streaming.py`` this module encodes a
policy specific to billed public APIs (the bytes-vs-latency dual-pass
tradeoff described below), not a reusable measurement primitive every
backend needs.
...
``Exchange`` here doubles as the ``aipt.backends.record.TurnExchange`` duck-typed
protocol object: every field that protocol requires (``wire_sent``, ``wire_recv``,
``req_payload_bytes``, ``resp_payload_bytes``, ``req_sent_ms``, ``ttfb_ms``,
``ttft_ms``, ``ttlt_ms``, ``turn_end_ms``, ``text``, ``request_json``,
``response_json``, ``error``) is present, so a gemini/openai adapter can hand one
straight to ``aipt.backends.record.turn_record`` without translation. The extra
fields (``status``, ``elapsed_ms``, ``response``) are adapter-internal convenience,
unused by the protocol but useful for building the next turn's history.
"""
...
from aipt.core.streaming import read_stream, since
