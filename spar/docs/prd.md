...
**산출물**:
- [x] 라우터 골드셋 JSONL (`data/goldsets/goldset.jsonl` — `expected_route` 필드 활용)
- [x] 영문 라우터 골드셋 JSONL (`data/goldsets/goldset_en.jsonl` — 8,540 문항, 영어 질의)
- [x] 라우터 평가 결과 리포트 (`output/router_eval/<ts>/router_eval_<layer>.md` + `.json` — 실패케이스 포함)
