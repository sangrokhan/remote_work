...
> **현 상태**: Phase 1 진행 중. LLM 모듈(factory/registry), 3-layer 라우터(Task 2.2), Milvus 클라이언트, 약어 사전(Task 1.6 ✅), FastAPI 앱, md ingest 파이프라인(Task 1.1/1.3 부분), embedder wrapper(Task 1.4 부분 — `EMBEDDING_TIMEOUT`/`EMBEDDING_BATCH_SIZE` env 지원, verbose 배치 진행 추적), encoder 싱글톤(Task 1.4 부분 ✅ — `ENCODER_MODEL`/`ENCODER_DEVICE` env vars), Codex+Gemini fallback 훅(INF-1b ✅), **LangGraph StateGraph 파이프라인** (`pipeline/` — Phase 5 조기 도입, reranker 첫 연결), **3GPP TS spec number 라우터(Task 2.2 부분 ✅ — RegexRouter TS 패턴 + spec intro ingest)**, **ingest acronym pre-pass + Milvus `keywords` ARRAY 필드 ✅** (Rel-18 2503 entries, noise filter 적용), **Milvus retrieval wiring ✅** (`retrieval/routing.py` + pipeline stub 제거 — rag_retrieve/structured_retrieve가 실제 `hybrid_search()` 호출), **멀티턴 history + 약어 컨텍스트 준비 (Task 2.5 부분 ✅** — `retrieval/query_rewriter.py`, `pipeline/prepare_context` 노드, API `history` 필드), **Task 1.7.2 평가 자동화 ✅** (Recall@K/MRR + faithfulness/answer_relevancy, `eval/ragas_metrics.py`), **라우터 골드셋 + 평가 스크립트 ✅** (`scripts/gen_router_goldset.py`, `scripts/run_router_eval.py` — Task 2.3), **통합 골드셋 생성기 ✅** (`scripts/gen_goldset.py` — QA+router 필드 단일 JSONL, 7개 QA 타입으로 6개 라우트 전체 커버, Codex→Gemini fallback, MAX_ATTEMPTS=10 백오프), **run_ingest --llm-url ✅** (약어 충돌 해소에 OpenAI-compatible LLM 사용), **Query Decomposition ✅ (Task 2.4** — `retrieval/query_decomposer.py`, `decompose`/`decomposed_retrieve` 노드, `RouteResult.needs_decomposition` 플래그) 구현됨, **LLM Query Rewriting ✅ (Task 2.5** — `retrieval/query_rewriter.py` `rewrite_query()`/`QueryRewriteResult`, `prompts/query_rewrite_system.txt`, `pipeline/rewrite_query` 노드, `tests/unit/retrieval/test_query_rewriter.py`) 구현됨, **Hybrid Verify Loop ✅ (Task 2.6** — `pipeline/tool_call` 노드(결정적 전략 fallback: decomposed→multi_hop→structured→rag + LLM 쿼리 재작성) + `pipeline/verify` 노드(LLM self-eval 1–5점, score&lt;3 시 재검색), `use_verify_loop` GraphConfig 플래그, `prompts/verify.txt`, `prompts/tool_call_rewrite.txt`, 최대 3회 retry) 구현됨, **Word(.docx) 파서 ✅ (Task 1.1 부분** — `parsers/docx_parser.py` DocxParser: heading 스타일 기반 섹션 추출, 표→CSV, 이미지→파일+.meta, placeholder 삽입, `parsers/docx_config.py` DocxParseConfig, `scripts/parse_docx.py` CLI) 구현됨, **DocxParser 버그픽스 ✅** (이미지+텍스트 혼합 단락 텍스트 보존, 병합셀 비인접 dedup, 한글 slug re.UNICODE, rel_id 중복 이미지 방지), **PDF 파서 ✅ (Task 1.1 부분** — `parsers/extractor/` 패키지: pipeline.py (`extract_pdf_to_outputs` 엔트리), text.py (body 추출/문단 병합), tables.py (`pdfplumber` 기반 표 추출+row 병합+continuation merge), images.py (embedded 이미지), font_profile.py (폰트 프로파일링), debug.py (page edge/rotated text/table drawing 디버그), shared.py (회전/세그먼트 병합 유틸)) 구현됨, **Counter Reference 파서 ✅ (Task 1.1 부분** — `parsers/counter_ref_parser.py`: 병합 셀 전파(`_expand_merged_cells`), 대/중 그룹 계층 carry-forward, `to_chunk_text()` RAG 지원, `scripts/ingest_counter_ref.py` Milvus ingest CLI, `router/regex_router.py` G-\d{3,5}/CELL.X.Y 패턴 STRUCTURED_LOOKUP, merge `2864f87f`) 구현됨, **Alarm Ref Excel parser + AlarmIndex ✅ (Task 1.1 부분** — `parsers/alarm_ref_parser.py` (AlarmRecord/parse_alarm_ref_excel), `retrieval/alarm_index.py` (AlarmIndex 싱글톤 + `get_alarm_index()`), `retrieval/routing.py::resolve_alarm_entity()` alarm_code 직접 lookup, `data/samples/alarm_excel_ref_sample.xlsx` 12행 샘플, `scripts/gen_alarm_sample.py`) 구현됨, **Pass B Excel Ingest ✅** — `scripts/run_ingest.py` `.xlsx` 파일 지원(`ingest_excel_file()`), 청크 keyword tagging에 Samsung 엔티티 포함, 단위/통합 테스트 추가), **영문 라우터 골드셋 ✅** (`data/goldsets/goldset_en.jsonl` — 8,540문항 영어 질의, Task 2.3), **라우터 eval 리포트 실패케이스+JSON ✅** (`scripts/run_router_eval.py` — `output/router_eval/<ts>/` MD+JSON, Task 2.3), **RegexRouter `_DEFINITION_RE` 확장 ✅** (`router/regex_router.py` — definition_explain recall 개선), **`get_all_keywords` dict 엔티티 처리 ✅** (`preprocessing/abbrev_mapper.py`).
...
│   ├── parsers/          # 문서 유형별 파서 — docx_parser.py (DocxParser), parameter_ref_parser.py, counter_ref_parser.py (병합 셀 대/중 그룹 계층), extractor/ (PDF 패키지) (Task 1.1 ✅ DOCX, ✅ PDF, ✅ Excel)
