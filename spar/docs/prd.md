...
- [x] **산출물**: `scripts/gen_goldset.py` — QA+router 필드 통합 단일 JSONL 생성기 (gen_goldset_qa + gen_router_goldset 통합; Codex→Gemini fallback, 지수 백오프 MAX_ATTEMPTS=10); 7개 QA 타입(terminology→definition_explain, technology/behavior→default_rag, diagnostic→diagnostic, procedural→procedural, comparative→comparative, lookup→structured_lookup) — 6개 라우트 전체 커버
