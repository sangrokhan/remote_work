...
- `--heading-profile <path>`: 외부 JSON의 `font_size -> h1~h6` 규칙으로 body markdown heading 추가
- `--raw <path>`: 선택 페이지 기준 문서 PDF base64만 저장하는 최소 raw dump 생성
- `--from-raw <path>`: raw dump 파일을 입력으로 읽어 기존 추출 파이프라인 실행
- `--page-write`: 디버그 목적의 페이지 구분 주석(`[//]: # (Page N)`)을 markdown 출력에 추가
...
python3 -m extractor sample.pdf \
  --out-md-dir artifacts/manual/md \
  --out-image-dir artifacts/manual/images \
  --heading-profile fixtures/font_heading_profile.sample.json
```
...
### 직접 실행 산출물
- `artifacts/manual/md/output.txt`: 첫 기술문서 기준 본문 텍스트
- `artifacts/manual/md/output.md`: 첫 기술문서 기준 본문 markdown
- `artifacts/manual/md/output_tables.md`: 첫 기술문서 기준 표 markdown
- `artifacts/manual/md/output_summary.json`: 추출 요약
- `artifacts/manual/md/output_font_profile.json`: body text의 `font_size + font_color` 조합 요약 (`--profile-fonts`)
- `artifacts/manual/md/output_font_profile.csv`: 동일 프로파일의 표 형태 출력 (`--profile-fonts`)
- `artifacts/manual/images/output_images/*`: body 영역 이미지 추출 결과
- `artifacts/manual/raw/sample.raw.dump`: 문서 전체 raw dump 예시 (`--raw`)

### Debug 산출물
- `--page-write`를 사용하면 본문 markdown과 table markdown에 `[//]: # (Page N)` 페이지 주석이 추가될 수 있습니다.
- `artifacts/manual/md/output_debug.json`: 표 구조 + 원본 drawing 객체 + 텍스트 폰트 크기 프로파일 디버그 (`--debug`)
- `artifacts/manual/md/output_edges_debug.json`: edge 분해 결과 디버그 (`--debug`)
- `artifacts/manual/md/output_watermark_debug.json`: 회전 문자 디버그 (`--debug-watermark`)
...
- `fixtures/font_heading_profile.sample.json`: `--heading-profile`용 최소 heading 규칙 샘플
...
8. `--heading-profile`이 있으면 외부 JSON의 `heading_rules[].match.font_size -> assign.tag/assign.markdown_prefix` 규칙으로 markdown heading prefix를 추가합니다.
...
## 산출물 예시 위치
- 텍스트/마크다운: `graph_pdf/artifacts/run_demo/md/FGR-BC0201.txt`, `FGR-BC0201.md`, `FGR-BC0201_tables.md`
- 이미지: `graph_pdf/artifacts/run_demo/images/FGR-BC0201_images/FGR-BC0201_image_1.png`
