...
|  | F1UPacketLossRateDL_QCI | This counter is the ratio of actually lost packets to all packets by the time statistics are collected for downlink GTP packets. A negative value denotes that the packets which were lost in the previous collection cycle, is an out-of-sequence inflow packet for the current collection cycle. |
