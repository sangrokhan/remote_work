...
TOOLS = [
    ...
    {"name": "search_nodes", "description": "Search schema nodes by a single keyword (one node NAME or word, e.g. 'interface' or 'mtu'). NOT for full schema paths - do not pass a slash path here, it will not match; if you already have a full schema path, call get_node instead. Use to DISAMBIGUATE when a name is vague or find_leaf returns several candidates; returns node_id + schema_path per hit. STEP 1 (disambiguation).", "inputSchema": {"type": "object", "properties": {"keyword": {"type": "string"}, "kind": {"type": "string"}, "top_k": {"type": "integer"}}, "required": ["keyword"]}},
    ...
    {"name": "get_node", "description": "Resolve a node by EITHER its node_id OR its full schema path (slash path). This is the tool to use when the user gives you a full path or you already know the path and need the node_id/metadata - do NOT use search_nodes for a path. Returns the node including its node_id.", "inputSchema": {"type": "object", "properties": {"node_id_or_path": {"type": "string"}}, "required": ["node_id_or_path"]}},
    ...
    {"name": "resolve_instance_path", "description": "Build the full keyed instance path for a node. STEP 3: call once you have all key_values from get_required_keys. Use the path string it returns verbatim; never write a path yourself.", "inputSchema": {"type": "object", "properties": {"node_id": {"type": "string"}, "key_values": {"type": "object"}}, "required": ["node_id", "key_values"]}},
