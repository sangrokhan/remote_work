...
`useWorkflowSocket.js`가 `window.location.hostname` + 포트 `10001`에서 런타임에 백엔드 URL을 자동 도출한다.

```
WS  : {ws|wss}://<브라우저 접속 호스트>:10001/ws/connect
REST: {http|https}://<브라우저 접속 호스트>:10001/graph
```

빌드 타임 env 변수(`VITE_WORKFLOW_WS_URL`, `VITE_WORKFLOW_GRAPH_URL`)는 더 이상 사용하지 않는다.  
외부 NAT 포트포워딩 환경에서 포트 10000과 함께 **포트 10001도 포워딩 필요**.
...
App.jsx 단일 파일에서 분리한 모듈 구조:

```
src/
├── App.jsx                  # 최상위 상태 조합
├── constants.js             # PANEL_WIDTH, MODEL_LIST, INITIAL_MESSAGES
├── components/
│   ├── ChatPane.jsx         # 메시지 목록 렌더링
│   ├── Composer.jsx         # 입력창 + 전송버튼 + 모델 선택
│   ├── MessageBubble.jsx    # 개별 말풍선
│   ├── PaneHeader.jsx       # 패널 헤더
│   ├── SettingsModal.jsx    # 설정 모달
│   └── WorkflowPanel.jsx    # 우측 워크플로우 패널
├── hooks/
│   ├── useWorkflowSocket.js # WS 연결, 메시지 처리, 그래프 폴백
│   ├── useWorkflowGraph.js  # Cytoscape 인스턴스 생성/파괴
│   └── useScrollBehavior.js # 자동 스크롤 + 스크롤바 표시 토글
└── utils/
    └── nodeUtils.js         # 노드 ID/라벨/클래스/색상 변환
```

분할 모드(`isSplitMode`) 활성 시 모델 선택 버튼 숨김. 전송 버튼은 `margin-left: auto`로 항상 우측 고정.
