...
retriever 도구가 milvus에서 `text`만 가져와 refiner가 doc 본문을 regex로 파싱해 `feature_id`/`feature_name`을 추출하던 구조를 doc metadata 직접 read로 전환.

- **수정**: `backend/tools/retriever.py`
  - 모든 milvus `output_fields=["text"]` → `["text", "feature_id", "feature_name"]` (BM25 / sparse_bge / sparse_meta / sparse_contents / dense_7 / dense_3 + dense-only 전략 1개)
  - `_filter_and_log_results` 반환 형식: `list[str]` → `list[dict]` (`{text, feature_id, feature_name}`), text 기준 dedup
  - `_apply_reranking` 시그니처 `list[str]` → `list[dict]`. 재랭킹 서비스에는 text만 페이로드, 응답 매핑 후 metadata 재첨부
- **수정**: `backend/langgraph_flow/agents/nodes/refiner_node.py`
  - `_extract_features_from_raw_results`: regex (`FGR-[A-Z]{2}\d{4}` / `"feature_name":\s*"..."`)  제거 → `item["feature_id"]` / `item["feature_name"]` 직접 read, dedup by id
- **호환성**: 기존 `_collect_retriever_results` / `_filter_excluded_docs` / `_format_results_for_refiner` 모두 dict-with-`text` 분기 이미 보유 → 변경 불필요
- **전제**: milvus collection schema에 `feature_id`, `feature_name` 스칼라 필드 존재해야 함 (없으면 ingest 파이프라인 보강 선행 필요)

### 9.6 executor enrichment prefix 라벨링 + 빈 값 제거 (2026-04-28)

`_execute_retrieve_subtask` safety-net enrichment이 placeholder 치환 실패 시 raw value만 prefix로 붙여 retriever가 어떤 값이 feature_id/feature_name인지 알 수 없었음. 또한 빈 문자열 value도 prefix에 포함되어 콤마 누적 발생.

- **수정**: `executor_node.py:311-318` `concrete` 딕셔너리 빌더에 `v.strip()` truthy 가드 추가 → 빈 값 배제
- **prefix 포맷**: `" ".join(values)` → `", ".join(f"{k}: {v}")` — 각 값에 binding key 라벨 부착 (e.g. `[feature_id: FGR-A, feature_name: Xn handover]`)
