...
- `scripts/run_ingest.py`: Phase 1 pre-pass(전체 파일 약어 수집) → Phase 2 ingest(map → chunk → keywords 주입) 파이프라인; `_find_chunk_keywords` — global + keywords 섹션 통합 탐색; Phase 2 루프 `[idx/N]` 파일 카운터 + embed 배치 verbose 진행 추적
