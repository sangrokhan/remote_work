...
- [x] 각 유형별 파서 개발
  - [x] Word .docx — `DocxParser`
  - [x] PDF — `parsers/extractor/` 패키지
  - [x] Counter Reference Excel — `CounterRecord` + `parse_counter_ref_excel()`
  - [x] Alarm Reference Excel — `AlarmRecord` + `parse_alarm_ref_excel()`
- [x] PDF→Markdown 변환 CLI
- [x] 3GPP TSpec-LLM fetcher
- [ ] OCR 스캔 문서 처리 (Tesseract, PaddleOCR)

**산출물**:
- [x] `parsers/docx_parser.py`, `parsers/docx_config.py`
- [x] `parsers/extractor/` (pipeline, text, tables, images, font_profile, notes, raw, shared, __main__)
- [x] `parsers/counter_ref_parser.py`, `parsers/alarm_ref_parser.py`
- [x] `data/samples/counter_ref_sample.xlsx`, `data/samples/alarm_excel_ref_sample.xlsx`
- [x] `scripts/convert_pdf_to_md.py`, `scripts/fetch_tspec_llm.py`
- [x] `scripts/ingest_counter_ref.py`, `scripts/gen_alarm_sample.py`
- [x] `router/regex_router.py` (counter/alarm/MO 패턴)
- [x] `retrieval/alarm_index.py`
- [x] `tests/parsers/test_counter_ref_parser.py`, `tests/parsers/test_alarm_ref_parser.py`
- [x] `tests/retrieval/test_alarm_index.py`, `tests/retrieval/test_routing_alarm.py`

**의존 라이브러리**: `pdfplumber`, `pypdf`, `Pillow`

### Task 1.2 — 메타데이터 스키마 정의 및 부착

- [x] 모든 청크에 메타데이터 강제 부착 (`src/spar/retrieval/milvus_client.py`):
...
- [ ] **Reference 문서 (Parameter/Counter/Alarm)**: 항목 단위 청크 (parameter 1개 = chunk 1개)
...
**산출물**:
- [x] `src/spar/ingest/chunkers.py`
- [ ] `chunkers/` 디렉토리 (유형별 모듈)
- [x] `scripts/run_ingest.py`
...
**산출물**:
- [x] `src/spar/retrieval/milvus_client.py` — `hybrid_search()` API
- [x] `src/spar/retrieval/routing.py` — Route→doc_type 매핑 + Milvus expr 빌더
- [x] `src/spar/ingest/embedder.py`
- [x] `src/spar/encoder/base.py`, `src/spar/encoder/registry.py`
...
- [x] Local CrossEncoderClient: sentence-transformers CrossEncoder 직접 로드
- [x] `RERANKER_BACKEND`(local|remote) / `RERANKER_DEVICE` env var 지원
- [x] LangGraph `rerank_node` 파이프라인 연결
- [ ] Latency 측정 및 batch 최적화

**산출물**:
- [x] `src/spar/reranker/client.py`, `config.py`, `factory.py`
- [x] `tests/reranker/` (18개 테스트)
- [ ] reranker 서빙 설정
...
- [x] BM25 인덱스에 동의어 확장 적용 (preprocess → expand_query → hybrid_search)
- [x] ingest 중 문서별 Abbreviations 섹션 자동 추출 → 사전 갱신
- [x] 청크별 포함 약어 목록을 Milvus `keywords` ARRAY 필드로 기록
- [x] Excel 파일 특정 column 값 추출 → `acronyms.json` `keywords` 섹션 병합
- [x] 질의 내 `keywords` 섹션 term 탐지 → `SparState.matched_terms`
- [x] Milvus `array_contains(keywords, term)` expr 필터

**산출물**:
- [x] `dictionary/acronyms.json`
- [x] `src/spar/preprocessing/abbrev_mapper.py`
- [x] `src/spar/ingest/excel_loader.py`
- [x] `src/spar/ingest/term_tagger.py`
- [x] `scripts/extract_acronyms.py`
- [x] `scripts/ingest_excel.py`

### Task 1.7 — 골드셋 구축 + Phase 1 평가

#### 1.7.1 Retrieval 골드셋 구축

- [ ] SME 협업 골드셋 100~200문항 작성
- [ ] 각 질의에 정답 청크 ID 또는 정답 문서/섹션 매핑
- [ ] 복잡 질의에 여러 relevant chunk 매핑
- [ ] 질의 관점별 균형 분포 (terminology / technology / behavior)
- [ ] `section` 필드: ground truth용 챕터 번호 (eval 전용)
...
- [x] 질의 유형별 분리 측정
- [x] eval_suite.py 멀티-config 비교 러너 (GraphConfig ablation)
- [x] graph.ainvoke() 기반 eval
- [ ] CI/CD 형태로 변경마다 자동 재측정

**산출물**:
- [x] `src/spar/eval/metrics.py`
- [x] `src/spar/eval/ragas_metrics.py`
- [x] `src/spar/eval/run_eval.py`
- [x] `src/spar/eval/run_ragas_eval.py`
- [x] `src/spar/eval/eval_suite.py`
- [x] `src/spar/eval/metrics_dashboard.md`
...
- [x] 라우트 정의 (`src/spar/router/schemas.py`):
...
**산출물**:
- [x] `src/spar/router/schemas.py`
- [x] `src/spar/router/routes_spec.md`
...
**산출물**:
- [x] `router/regex_router.py`
- [x] `scripts/slice_3gpp_intros.py`
...
**산출물**:
- [x] `router/embedding_router.py`
- [ ] `router/route_examples.yaml`
...
**산출물**:
- [x] `router/llm_router.py`
- [x] `router/hybrid_router.py`
- [x] `src/spar/prompts/router_system.txt`
...
**산출물**:
- [ ] `data/goldsets/goldset.jsonl` (gen_goldset.py 실행 필요)
- [ ] `data/eval_results/router_eval_{date}.md` (run_router_eval.py 실행 필요)
- [x] `scripts/gen_goldset.py`
- [x] `scripts/run_router_eval.py`
...
**산출물**:
- [x] `src/spar/retrieval/query_decomposer.py`
- [x] `src/spar/prompts/decompose_system.txt`
...
**산출물**:
- [x] `src/spar/retrieval/query_rewriter.py`
...
**산출물**:
- [x] `src/spar/pipeline/nodes.py` (verify_node, tool_call_node)
- [x] `src/spar/pipeline/graph.py` (conditional edges)
- [x] `src/spar/pipeline/state.py` (verify_score field)
- [x] `src/spar/pipeline/config.py` (verify_loop preset)
- [x] `src/spar/prompts/verify.txt`, `src/spar/prompts/tool_call_rewrite.txt`
- [x] `tests/pipeline/test_verify_node.py`
...
**산출물**:
- [ ] `retrieval/multi_index_search.py`
...
**산출물**:
- [ ] `generation/citation_enforcer.py`
- [ ] 답변 형식 spec
...
**산출물**:
- [ ] `generation/confidence_scorer.py`
...
**산출물**:
- [x] `src/spar/pipeline/graph.py`
- [x] `src/spar/pipeline/nodes.py`
- [x] `src/spar/pipeline/state.py`
- [x] `src/spar/pipeline/config.py`
...
**산출물**:
- [x] `scripts/serve_vllm.sh`
- [x] `src/spar/llm/` (client, config, factory, registry)
- [x] `src/spar/api/app.py`
- [ ] 서빙 설정, 부하 테스트 결과
...
**산출물**:
- [x] `src/spar/retrieval/milvus_client.py`
- [x] `configs/milvus/`
- [x] `scripts/init_milvus.py`
- [ ] 인프라 구성도
