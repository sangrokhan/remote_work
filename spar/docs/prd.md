...
- [x] **버그픽스** (2026-05-02): 이미지+텍스트 혼합 단락 텍스트 소실, 병합셀 비인접 중복, 한글 slug re.UNICODE, 동일 rel_id 이미지 중복 저장 수정
...
- [x] **산출물** (2026-05-02): `parsers/counter_ref_parser.py` — CounterRecord, parse_counter_ref_excel(); 병합 셀 전파(_expand_merged_cells), value_range 분리, 한/영 헤더 별칭 지원
- [x] **산출물** (2026-05-02): `data/samples/counter_ref_sample.xlsx` — 병합 셀 포함 13개 카운터 샘플 (RRC/MAC/PHY 3개 대그룹, 5개 중그룹)
- [x] **산출물** (2026-05-02): `tests/parsers/test_counter_ref_parser.py` — 29개 테스트 (병합 셀 전파, 빈 행 스킵, 한국어 헤더, value_range 분리, 샘플 파일 검증)
- [x] **산출물** (2026-05-02): `scripts/ingest_counter_ref.py` — Excel → Milvus ingest CLI (--dry-run/--force/--sheet/--product/--release)
- [x] **산출물** (2026-05-02): `router/regex_router.py` — G-\d{3,5} (conf 1.0) / CELL.X.Y (conf 0.95) 패턴 → STRUCTURED_LOOKUP 추가
- **완료**: 2026-05-02 — feat/counter-ref-parser → main merged (`2864f87f`)
- [x] **Alarm Reference Excel parser** (2026-05-02 — branch `feat/alarm-ref-sample-and-parser`)
  - `parsers/alarm_ref_parser.py` — `AlarmRecord` (alarm_id/alarm_name/severity/category/module/pdf_ref) + `parse_alarm_ref_excel()` (헤더 별칭, alarm_id 대문자 정규화, `to_chunk_text()` / `to_keywords()` / `to_dict()`)
  - `retrieval/alarm_index.py` — `AlarmIndex` 싱글톤 + `get_alarm_index()` (env `SPAR_ALARM_REF_PATH` 우선, 기본 샘플 fallback, `lookup()` 대소문자 무관, `search_by_name()` 부분일치)
  - `retrieval/routing.py::resolve_alarm_entity()` — `alarm_code` 엔티티 → AlarmIndex 직접 lookup → `{alarm_id, alarm_name, severity, category, module, pdf_ref, keywords}` 반환 (STRUCTURED_LOOKUP 단축 경로)
  - `data/samples/alarm_excel_ref_sample.xlsx` — 12행 샘플 (ALM-1001..ALM-1012, Critical/Major/Minor/Warning, Radio/Transport/HW/SW, gNB-DU/gNB-CU/RU/BBU/OAM)
  ...
  - 후속: alarm Reference PDF 파서 (alarm_id를 join 키로 설명/조치 본문 결합)
...
- [x] sentence-transformers embedder wrapper (`src/spar/ingest/embedder.py`) — `verbose` param: remote path `[done/N]\r` 배치 진행, local path `show_progress_bar` 활성화
...
- `scripts/run_ingest.py`: Phase 1 pre-pass(전체 파일 약어 수집) → Phase 2 ingest(map → chunk → keywords 주입) 파이프라인; `_find_chunk_keywords` — global + keywords 섹션 통합 탐색; Phase 2 루프 `[idx/N]` 파일 카운터 + embed 배치 verbose 진행 추적; `--llm-url`/`--llm-model` 플래그로 약어 충돌 해소에 LLM 사용 가능 (OpenAI-compatible 엔드포인트)
...
- [x] 라우터 전용 골드셋 (QA 7개 타입 → 6개 route 매핑) — `scripts/gen_goldset.py`
- [x] Confusion matrix 분석 — `scripts/run_router_eval.py` 출력에 포함
- [x] 라우팅 정확도 측정 (overall + per-route) — `scripts/run_router_eval.py`
- [ ] **산출물**: `data/goldsets/goldset.jsonl` ← gen_goldset.py 실행 후 생성
- [ ] **산출물**: `data/eval_results/router_eval_{date}.md` ← run_router_eval.py 실행 후 생성
- [x] **산출물**: `scripts/gen_goldset.py` — QA+router 통합 단일 JSONL 생성기; 7개 QA 타입(terminology→definition_explain, technology/behavior→default_rag, diagnostic→diagnostic, procedural→procedural, comparative→comparative, lookup→structured_lookup), 6개 라우트 전체 커버, Codex→Gemini fallback, MAX_ATTEMPTS=10
- [x] **산출물**: `scripts/run_router_eval.py`
- ~~`scripts/gen_goldset_qa.py`~~ — gen_goldset.py로 통합 후 삭제
- ~~`scripts/gen_router_goldset.py`~~ — gen_goldset.py로 통합 후 삭제
...
> ✅ **완료** (2026-05-01) — feat/query-rewriter-llm branch

- [x] 멀티턴 대화에서 질의를 self-contained 형태로 rewrite
- [x] 약어 자동 확장 — dictionary 기반 관련 약어 추출 → LLM 프롬프트 context 제공
- [x] 모호한 지시어 명시화 ("그 파라미터" → 실제 이름)
- [x] **산출물**: `src/spar/retrieval/query_rewriter.py` — `QueryRewriteResult` + `rewrite_query()`
- [x] **산출물**: `src/spar/prompts/query_rewrite_system.txt`
- [x] **산출물**: `src/spar/pipeline/nodes.py` — `rewrite_query` node
- [x] **산출물**: `tests/unit/retrieval/test_query_rewriter.py`
- [x] **부가 산출물**: `QueryRequest.history`, `SparState.history/history_context`, `pipeline/prepare_context` 노드

**완료**: 2026-05-01 — feat/query-rewriter-llm branch

### Task 2.6 — Hybrid Verify Loop (tool_call + verify nodes)

> ✅ **완료** (2026-05-02) — feat/hybrid-verify-loop branch

- [x] Verify node: LLM self-evaluation score (1-5) on retrieved context + query
- [x] Tool_call node: Deterministic strategy fallback (decomposed → multi_hop → structured → rag) + LLM query rewrite on low confidence
- [x] Wire both nodes into LangGraph conditional routing
- [x] Activate via `use_verify_loop=True` in GraphConfig preset (verify_loop)
- [x] Max 3 retries per query with backoff
- [x] LLM score threshold logic: if < 3 → fallback strategy selection
- [x] **산출물**: `src/spar/pipeline/nodes.py` (verify_node, tool_call_node), `src/spar/pipeline/graph.py` (conditional edges), `src/spar/pipeline/state.py` (verify_score field), `src/spar/pipeline/config.py` (verify_loop preset)
- [x] **산출물**: `src/spar/prompts/verify.txt`, `src/spar/prompts/tool_call_rewrite.txt`
- [x] **산출물**: `tests/pipeline/test_verify_node.py` (59개 테스트 통과)

**완료**: 2026-05-02 — feat/hybrid-verify-loop branch (merge commit: 95604589)
